...
SL_WEAK void app_process_action(void)
{
  ...
  if(button_pressed){
      ...
      QASM_DISPATCH(QMsm_bt_SPC51_p, &buttonEvt.super, (void)0U);
...
// function call back for application reply if sleeping is okay
bool app_is_ok_to_sleep(void)
{
  if (blink_expired | update_timer_expired | button_pressed)
    return false;
  else
    return true;
}
