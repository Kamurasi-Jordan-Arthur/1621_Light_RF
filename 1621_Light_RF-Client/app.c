...
#define CHARACTERISTIC_INVALID        ((uint8_t)0x00u)
...
//  int8_t   remote_tx_power;
  ...
  uint16_t led_num_characteristic_handle;
  uint16_t percentage_characteristic_handle;
  uint16_t status_characteristic_handle;
  uint16_t send_characteristic_handle;
  uint8_t  percentage;
  uint8_t  led_num;
  uint8_t  status;
...
static const uint8_t led_service[16] =  {
...
//};
//TODO the service, above has to be updated


// Temperature Measurement characteristic UUID defined by Bluetooth SIG
//static const uint8_t led_char_UUID[2] = { 0x1c, 0x2a };
//static const uint8_t percentage_char_UUID[2] = { 0x1c, 0x2a };
//static const uint8_t status_char_UUID[2] = { 0x1c, 0x2a };
...
static const uint8_t percentage_char_UUID[16] = {
...
//TODO the UUIDs above have to be updated.
//button change indication
...
static uint8_t service_InAdvertisement(uint8_t *data, uint8_t len);
...
SL_WEAK void app_init(void)
{
  ...
  init_properties();
  app_log_info("bt_client initialized.\n" APP_LOG_NL);
  ...
  //  if (buttonChanged){
    if (0U){
      ...
      if (sl_simple_button_get_state(&sl_button_btn0) == SL_SIMPLE_BUTTON_PRESSED){
          //write the values to the sever gatt table

          sc = sl_bt_gatt_write_characteristic_value(
              conn_properties[0].connection_handle,
              conn_properties[0].led_num_characteristic_handle,
              sizeof(conn_properties[0].led_num),
              &(conn_properties[0].led_num));
          app_assert_status(sc);

          sc = sl_bt_gatt_write_characteristic_value(
              conn_properties[0].connection_handle,
              conn_properties[0].percentage_characteristic_handle,
              sizeof(conn_properties[0].percentage),
              &(conn_properties[0].percentage));
          app_assert_status(sc);

          sc = sl_bt_gatt_write_characteristic_value(
              conn_properties[0].connection_handle,
              conn_properties[0].status_characteristic_handle,
              sizeof(conn_properties[0].status),
              &(conn_properties[0].status) );

          app_assert_status(sc);


          do {
              conn_properties[0].status = conn_properties[0].status ^ (uint8_t)1U;
              conn_properties[0].led_num++;
              conn_properties[0].percentage++;
          } while (0);
          app_log_info("Data written");
  ...
  switch (SL_BT_MSG_ID(evt->header)) {
    ...
    case sl_bt_evt_scanner_legacy_advertisement_report_id:
      ...
      if (evt->data.evt_scanner_legacy_advertisement_report.event_flags
        ...
        // If a Led_control  advertisement is found...\n
        if (service_InAdvertisement(&(evt->data.evt_scanner_legacy_advertisement_report.data.data[0]),
                                          evt->data.evt_scanner_legacy_advertisement_report.data.len) != 0) {

//            addr_value = evt->data.evt_scanner_legacy_advertisement_report.address[1]<<8 + evt->data.evt_scanner_legacy_advertisement_report.address[0];
//            for (uint8_t i = 0; i < active_connections_num; i ++){
//                if (conn_properties[i].server_address == addr_value){
//                    // denay any made connections
//                    break;
//                }
//            }
          ...
          // then stop scanning for a while
            ...
            if (conn_state == scanning && active_connections_num < SL_BT_CONFIG_MAX_CONNECTIONS) {
    ...
    case sl_bt_evt_connection_opened_id:
      // Discover Led service on the responder device
...
//      app_assert_status(sc);
      ...
      //TODO Check out the power reporting thing.

      conn_state = discovering_service;
      app_log_append_info("Connection established.\nDiscovering_service...\n");
...
//
//          app_log_info("Discovered UUID: ");
//          for (int i = 0; i < evt->data.evt_gatt_service.uuid.len; i++) {
//            app_log_append_info("0x%02x", evt->data.evt_gatt_service.uuid.data[i]);
////            app_log_append_info("0x%02x", led_service[i]);
//          }
//
//          app_log_append_info("\n");
          ...
          if(memcmp(evt->data.evt_gatt_service.uuid.data, led_service, sizeof(led_service)) == 0){

               conn_properties[table_index].service_handle = evt->data.evt_gatt_service.service;
    ...
    case sl_bt_evt_gatt_characteristic_id:
      ...
      if (table_index != TABLE_INDEX_INVALID) {
          ...
          if (memcmp(evt->data.evt_gatt_characteristic.uuid.data, led_char_UUID, sizeof(percentage_char_UUID)) == 0) {
              conn_properties[table_index].led_num_characteristic_handle = evt->data.evt_gatt_characteristic.characteristic;
              app_log_info("led_char Found\n");

          }
          else if (memcmp(evt->data.evt_gatt_characteristic.uuid.data, percentage_char_UUID, sizeof(percentage_char_UUID)) == 0) {
              conn_properties[table_index].percentage_characteristic_handle = evt->data.evt_gatt_characteristic.characteristic;
              app_log_info("Percentage_char Found\n");


          }
          else if (memcmp(evt->data.evt_gatt_characteristic.uuid.data, status_char_UUID, sizeof(status_char_UUID)) == 0) {
              conn_properties[table_index].status_characteristic_handle = evt->data.evt_gatt_characteristic.characteristic;
              app_log_info("Status_char Found\n");

//              sc = sl_bt_gatt_read_characteristic_value( evt->data.evt_gatt_characteristic.connection,  evt->data.evt_gatt_characteristic.characteristic);


          }else if (memcmp(evt->data.evt_gatt_characteristic.uuid.data, send_char_UUID, sizeof(send_char_UUID)) == 0) {
              conn_properties[table_index].send_characteristic_handle = evt->data.evt_gatt_characteristic.characteristic;
              app_log_info("Send_char Found.\n\n");

          }
          else {
              app_log_info("Invalid char UUID.\n");
          }

          // Get notified on changes in the characteristic
...
//          app_assert_status(sc);
      ...
      app_log_info("unknown connection\n");
    ...
    case sl_bt_evt_gatt_procedure_completed_id:
      ...
      if (table_index == TABLE_INDEX_INVALID) {
          app_log_info("Invalid_ connection.");
        break;
      }
      // If service discovery finished
      if (conn_state == discovering_service
          && conn_properties[table_index].service_handle) {

        // Discover thermometer characteristic on the responder device
//          sc = sl_bt_gatt_discovering_characteristics_by_uuid(evt->data.evt_gatt_procedure_completed.connection,
//                                                                   conn_properties[table_index].service_handle,
//                                                                   sizeof(led_char_UUID),
//                                                                   (const uint8_t*)led_char_UUID);
//          app_assert_status(sc);
//
//          sc = sl_bt_gatt_discovering_characteristics_by_uuid(evt->data.evt_gatt_procedure_completed.connection,
//                                                                   conn_properties[table_index].service_handle,
//                                                                   sizeof(percentage_char_UUID),
//                                                                   (const uint8_t*)percentage_char_UUID);
//          app_assert_status(sc);
//
//          sc = sl_bt_gatt_discovering_characteristics_by_uuid(evt->data.evt_gatt_procedure_completed.connection,
//                                                                   conn_properties[table_index].service_handle,
//                                                                   sizeof(status_char_UUID),
//                                                                   (const uint8_t*)status_char_UUID);
//          app_assert_status(sc);
          ...
          conn_state = discovering_characteristics;
          app_log_info("discovering_characteristics...\n");
          break;
      }
      // If characteristic discovery finished
      if (conn_state == discovering_characteristics
          && conn_properties[table_index].percentage_characteristic_handle
          && conn_properties[table_index].led_num_characteristic_handle
          && conn_properties[table_index].status_characteristic_handle
          && conn_properties[table_index].send_characteristic_handle) {
        // stop discovering
//        sc = sl_bt_scanner_stop();
        // enable indications
          if(count <= 3U){
              if (count == (uint8_t)0U){
                  ...
                  count++;
                  break;
              }else if(count == (uint8_t)1U){
                  app_log_info("LED - Notification Enabled\n");
        ...
        if (active_connections_num == (uint8_t)1U){
        ...
        if (active_connections_num < SL_BT_CONFIG_MAX_CONNECTIONS) {
              ...
              conn_state = scanning;
              app_log_info("Running connection %d\n" , active_connections_num);
              app_log_info("Re-scanning...\n");
      ...
      // If indication ID finished just break
      app_log_info("Unhandled result %d\n", conn_state);
      break;
...
//
//
//         case (const int8_t)(conn_properties[table_index].led_num_characteristic_handle):
//
//           conn_properties[table_index].led_num = evt->data.evt_gatt_characteristic_value.value.data[0] ;
...
//
//           app_log_info("led_value received: %d",conn_properties[table_index].led_num);
//
//           break;
//
//         case (const int8_t)(conn_properties[table_index].percentage_characteristic_handle):
//
//         conn_properties[table_index].percentage = evt->data.evt_gatt_characteristic_value.value.data[0] ;
////                   sl_bt_gatt_read_characteristic_value(
////                       evt->data.evt_gatt_characteristic_value.connection,
////                       evt->data.evt_gatt_characteristic_value.characteristic);
//
//         app_log_info("percentage_value received: %d",conn_properties[table_index].percentage );
//
//           break;
//
//         case (const int8_t)(conn_properties[table_index].status_characteristic_handle):
//
...
////                       evt->data.evt_gatt_characteristic_value.characteristic);
//
//         app_log_info("status value received: %d",conn_properties[table_index].status );
//
//           break;
//
//         default:
//           app_log_info("Unknown characteristic");
//           break;
//       }

      if(evt->data.evt_gatt_characteristic_value.characteristic == conn_properties[table_index].led_num_characteristic_handle){
          conn_properties[table_index].led_num = evt->data.evt_gatt_characteristic_value.value.data[0] ;
//          app_log_info("Led_value received: %d\n",conn_properties[table_index].led_num);
          count = 1U;

      }else if(evt->data.evt_gatt_characteristic_value.characteristic == conn_properties[table_index].percentage_characteristic_handle){
          conn_properties[table_index].percentage = evt->data.evt_gatt_characteristic_value.value.data[0] ;
//          app_log_info("Percentage_value received: %d\n",conn_properties[table_index].percentage);
          count = 2U;
//          printf("\rReceived Led configuration : L%02d %02d%% %02d",
//                            conn_properties[table_index].led_num,
//                            conn_properties[table_index].percentage,
//                            conn_properties[table_index].status);
//          app_log_info("Percentage count %d\n", count);

      }else if(evt->data.evt_gatt_characteristic_value.characteristic == conn_properties[table_index].status_characteristic_handle ){
          conn_properties[table_index].status = evt->data.evt_gatt_characteristic_value.value.data[0] ;
//          app_log_info("Status_value received: %d\n",conn_properties[table_index].status);
          count = 3U;
...
static void init_properties(void)
{
  ...
  for (i = 0; i < SL_BT_CONFIG_MAX_CONNECTIONS; i++) {
    ...
    conn_properties[i].service_handle = SERVICE_HANDLE_INVALID;
    conn_properties[i].led_num_characteristic_handle = CHARACTERISTIC_HANDLE_INVALID;
    conn_properties[i].percentage_characteristic_handle = CHARACTERISTIC_HANDLE_INVALID;
    conn_properties[i].status_characteristic_handle = CHARACTERISTIC_HANDLE_INVALID;
    conn_properties[i].led_num = CHARACTERISTIC_INVALID;
    conn_properties[i].percentage = CHARACTERISTIC_INVALID;
    conn_properties[i].status = CHARACTERISTIC_INVALID;
...
//static uint8_t service_InAdvertisement(uint8_t *data, uint8_t len)
////todo the ad_field_type!! and why type? is an issue.
//{
...
//      // compare UUID to Led service UUID
//      if (memcmp(&data[i + 2], led_service, 2) == 0) {
//        return 1;
//      }
//    }
...
static uint8_t service_InAdvertisement(uint8_t *data, uint8_t len) {
  ...
  while (i < len) {
    ...
    if (ad_field_type == 0x06 || ad_field_type == 0x07) {
      ...
      if (memcmp(&data[i + 2], led_service, sizeof(led_service)) == 0) {
...
static uint8_t find_index_by_connection_handle(uint8_t connection)
{
  for (uint8_t i = 0; i < active_connections_num; i++) {
    ...
    }else if(conn_properties[i].connection_handle == CONNECTION_HANDLE_INVALID){
...
static void remove_connection(uint8_t connection)
{
  ...
  if(table_index != TABLE_INDEX_INVALID){
      ...
      if (active_connections_num > 0) {
        ...
        // disable button when there is no running connection
//        if (!active_connections_num){
...
//        }
      ...
      for (i = table_index; i <= active_connections_num; i++) {
          //todo
          //watch out for that i <= active_con...\n
